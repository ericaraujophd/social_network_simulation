...
to setup
  ...
  setup-globals
  generate-network
  ;if graphics? [layout-equidistant 50]
  reset-ticks
end
...
to generate-posts
  ask users [
    if random-float 1 <= prob-posting [
      ; generates a post
      hatch-posts 1 [
...
to generate-network
  nw:generate-small-world users links 2 10 2.0 false [
    ...
    ifelse random-float 1 <= (perc-public-groups / 100) [
      set public-group? true
      ; group 0 - leftists
      ; group 1 - centrists
      ; group 2 - rightists
      ifelse user-positioning <= -0.3 [ set id-group 0 ][
        ifelse user-positioning >= 0.3 [ set id-group 2 ][ set id-group 1 ]
      ]
    ][
      set public-group? false
    ]
...
to layout-random
  ask users [
    ...
    ifelse (spot != nobody) [ move-to spot ][print "No space for all users!"]
  ]
end
